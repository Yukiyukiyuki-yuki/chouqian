...
const THRESHOLD = 120; // 需要持续摇 ~5-8 秒
const ENERGY_PER_MOTION = 3.5; // 每帧能量增加
const ENERGY_DECAY = 0.18; // 慢衰减，停下来才掉
...
function CameraShakeLots({ onDrawn }) {
  ...
  const dirRef = useRef(0); // -1 to 1, motion direction (left/right)
  const magRef = useRef(0); // current motion magnitude
  const sticksRef = useRef(
    Array.from({length:STICK_COUNT}, (_,i) => ({
      angle: 0,           // current angle
      velocity: 0,        // angular velocity
      offset: (i - STICK_COUNT/2) * 0.3, // slight spread
      damping: 0.82 + Math.random() * 0.08, // each stick has different damping
  ...
  const [flyingStick, setFlyingStick] = useState(null); // index of the stick that flies out
    ...
    const tick=()=>{
      ...
      if(video.readyState>=2){
        ...
        if(prevFrameRef.current){
          ...
          let direction=0;
          if(total>500){
            direction = -(rightDiff-leftDiff)/(total); // 镜像反转
          }

          // 运动幅度 0~1
          const magnitude = Math.min(totalDiff / 40000, 1);

          dirRef.current = dirRef.current * 0.6 + direction * 0.4; // smoothing
          magRef.current = magnitude;
          setMotionDir(dirRef.current);
          setMotionMag(magnitude);

          // 累积能量（只有运动幅度够大才算）
          if(magnitude > 0.02){
            energyRef.current = Math.min(energyRef.current + magnitude * ENERGY_PER_MOTION, THRESHOLD);
          }
        }
      ...
      // 能量衰减
      energyRef.current = Math.max(energyRef.current - ENERGY_DECAY, 0);
      ...
      const force = dir * mag * 35; // 方向力

      const newAngles = sticks.map((s,i)=>{
        // 弹簧力（回正） + 外力（手势方向） + 随机抖动
        const spring = -s.stiffness * s.angle;
        const randomJitter = mag > 0.05 ? (Math.random()-0.5) * mag * 8 : 0;
        const externalForce = force + randomJitter;

        s.velocity += spring + externalForce * 0.3;
      ...
      if(energyRef.current >= THRESHOLD){
        ...
        setDone(true);
        // 随机选一根签飞出
        const flyIdx = Math.floor(Math.random() * STICK_COUNT);
        setFlyingStick(flyIdx);
        const lot = LOT_POEMS[Math.floor(Math.random()*LOT_POEMS.length)];
        setTimeout(()=>{stopCam();onDrawn(lot);},1200);
        return;
      }
    ...
    <div>
      <div className="lots-scene">
        ...
        <div className="lots-overlay">
          <div className="lots-hint">📹 对着摄像头持续挥动双手摇签</div>

          {/* 签筒 */}
          <div className="lots-sticks">
            {stickAngles.map((a,i)=>(
              <div key={i}
                className={`ls ${flyingStick===i?"ls-fly":""}`}
                style={{
                  transform: flyingStick===i
                    ? `rotate(${a+60}deg) translateY(-200px)`
                    : `rotate(${a}deg)`,
                  opacity: flyingStick===i ? 0 : 1,
                  transition: flyingStick===i ? 'all 0.8s ease-out' : 'transform 0.05s',
                }}
              />
            ))}
          </div>
          ...
          {motionMag>0.05 && !done && (
            <div className="motion-arrow" style={{
              transform:`translateX(${motionDir*60}px) scale(${0.5+motionMag})`,
              opacity: motionMag*2,
            }}>
...
function MouseShakeLots({onDrawn}){
  ...
  useEffect(()=>{
    ...
    const handler=e=>{
      if(e.code==="Space"){
        ...
        setEnergy(energyRef.current);
        if(energyRef.current>=THRESHOLD){
          setDone(true);setFlyingStick(Math.floor(Math.random()*STICK_COUNT));
          setTimeout(()=>onDrawn(LOT_POEMS[Math.floor(Math.random()*LOT_POEMS.length)]),1200);
        }
      }
    };
  ...
  const onMove=useCallback((x,y)=>{
    ...
    if(motion>0.02){
      ...
      energyRef.current=Math.min(energyRef.current+motion*ENERGY_PER_MOTION*1.2,THRESHOLD);
      setEnergy(energyRef.current);
      const newAngles=sticksRef.current.map(s=>{
  ...
  return(<div>
    <div className="lots-scene" ref={zoneRef} style={{cursor:done?"default":"none"}}
      ...
      <div className="lots-overlay">
        ...
        <div className="lots-sticks">
          {stickAngles.map((a,i)=>(
            <div key={i} className={`ls ${flyingStick===i?"ls-fly":""}`}
              style={{transform:flyingStick===i?`rotate(${a+60}deg) translateY(-200px)`:`rotate(${a}deg)`,
                opacity:flyingStick===i?0:1,
                transition:flyingStick===i?'all 0.8s ease-out':'transform 0.05s'}}/>
          ))}
        </div>
